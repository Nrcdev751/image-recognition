  ...
  return (
    <div className="p-8 lg:p-12 flex flex-col flex-grow h-full">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white">Database Registry</h2>
          <p className="text-slate-500 text-xs">Total subjects ready for comparison: {users.length}</p>
        </div>
        <button 
          onClick={onAddUser}
      ...
      <div className="mt-auto pt-8 flex gap-4">
        <button 
          ...
          className={`flex-grow py-4 rounded-2xl font-black uppercase tracking-widest transition-all ${
            users.length > 0 ? 'bg-white text-slate-950 hover:bg-cyan-400 shadow-xl' : 'bg-slate-800 text-slate-600 cursor-not-allowed'
          }`}
        >
          Enter Scanning Mode
        </button>
